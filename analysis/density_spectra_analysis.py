...
model = load_model(directory_loc + 'ANN_1DTISE/density_spectra/saved_model')
...
print(energy_error)
print(np.mean(np.abs(energy_error)))
...
for i in range(4):
    ...
    true_IR = test_y_tf[:,i+1] - test_y_tf[:,0]
...
x = np.linspace(0,np.max(test_y[:,4])+1,1000)
y = x
...
# plt.show()
plt.savefig(directory_loc + 'ANN_1DTISE/Images_2/den_energies.tiff',dpi=1200)
