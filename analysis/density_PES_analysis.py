...
model = load_model(directory_loc + 'ANN_1DTISE/density_PES/saved_model')
...
plt.show()
# plt.savefig(directory_loc + 'ANN_1DTISE/Images_2/den_PES_ASW.tif',dpi=1200)
...
for j in range(8):
    ...
    u50_error_tracker = []
    for i in range(q):
        # predictions_val_smooth_u50_idx = np.where(predictions_val_smooth[i]<etest_y[i,j])[0]
        predictions_val_smooth_u50_idx = np.where(predictions_val_smooth[i]<(((j+1)*10)/627))[0]
        predictions_val_smooth_u50 = predictions_val_smooth[i,predictions_val_smooth_u50_idx]
        val_y_u50 = test_y[i,predictions_val_smooth_u50_idx]
        u50_error_tracker.append(np.mean(predictions_val_smooth_u50-val_y_u50)*627)
