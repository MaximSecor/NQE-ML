...
for q in range(4):
    ...
    train_X, test_X, train_y, test_y = train_test_split(target_wave[:,:,0]**2, target_wave[:,:,i]**2, test_size = 0.1, random_state = 1103)
    ...
    model = load_model(directory_loc + 'ANN_1DTISE/density_density/DENS_'+str(int(i))+'/saved_model')
...
train_X, val_X, train_y, val_y = train_test_split(features_potential, target_energies, test_size = 0.1, random_state = 1103)
train_X, gsden_X, train_y, val_y = train_test_split(features, target_energies, test_size = 0.1, random_state = 1103)
...
for i in range(1000):
    ...
    # plt.plot(domain,gsden_X[i]*scale_den+add[0],'m', label='FGH')    
    # plt.plot(domain,density_true[0,i]*scale_den+add[1],'r',domain,density_pred[0,i]*scale_den+add[1],'--b')
    # plt.plot(domain,density_true[1,i]*scale_den+add[2],'r',domain,density_pred[1,i]*scale_den+add[2],'--b')
    # plt.plot(domain,density_true[2,i]*scale_den+add[3],'r',domain,density_pred[2,i]*scale_den+add[3],'--b')
    # plt.plot(domain,density_true[3,i]*scale_den+add[4],'r',domain,density_pred[3,i]*scale_den+add[4],'--b')
    ...
    temp = []
    temp.append(np.sum(np.abs(density_true[0,i]-density_pred[0,i])))
    temp.append(np.sum(np.abs(density_true[1,i]-density_pred[1,i])))
    temp.append(np.sum(np.abs(density_true[2,i]-density_pred[2,i])))
    temp.append(np.sum(np.abs(density_true[3,i]-density_pred[3,i])))
    temp = np.array(temp)
    
    print(np.mean(temp))
    
    temp_2.append(np.sum(temp))
    
#%%
    
temp_2 = np.array(temp_2)
best_1 = np.where((temp_2)<0.4)[0]
best_2 = np.where((temp_2)>0.2)[0]
...
for q in range(len(best)):
    ...
    density_pred_smooth_0 = running_mean(density_pred[0,i],N)
    density_pred_smooth_1 = running_mean(density_pred[1,i],N)
    density_pred_smooth_2 = running_mean(density_pred[2,i],N)
    density_pred_smooth_3 = running_mean(density_pred[3,i],N)
    ...
    plt.plot(domain,val_X[i]*627,'k', label='Potential')
    plt.plot(domain,gsden_X[i]*scale_den+add[0],'m', label='FGH')    
    plt.plot(domain,density_true[0,i]*scale_den+add[1],'r',domain[start:finish],density_pred_smooth_0*scale_den+add[1],'--b')
    plt.plot(domain,density_true[1,i]*scale_den+add[2],'r',domain[start:finish],density_pred_smooth_1*scale_den+add[2],'--b')
    plt.plot(domain,density_true[2,i]*scale_den+add[3],'r',domain[start:finish],density_pred_smooth_2*scale_den+add[3],'--b')
    plt.plot(domain,density_true[3,i]*scale_den+add[4],'r',domain[start:finish],density_pred_smooth_3*scale_den+add[4],'--b')
...
# plt.show()
plt.savefig(directory_loc + 'ANN_1DTISE/Images_2/den_den_ASW.tif',dpi=1200)
...
plt.show()
# plt.savefig(directory_loc + 'ANN_1DTISE/Images/den_den_HO.tif',dpi=1200)
...
# plt.show()
plt.savefig(directory_loc + 'ANN_1DTISE/Images_2/den_den_SDW.tif',dpi=1200)
...
plot4[0] = np.sum(domain*val_X,1)
plot4[1] = np.sum(domain*val_X,1)
...
plt.show()
# plt.savefig(directory_loc + 'ANN_1DTISE/Images/density_density_position_expectation.tif',dpi=1200)
